...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        source.d = 0
        start = NodeDistancePair(source, source.d)
        Q.insert(start)
        node_to_pair[source] = start

        for node in nodes:
            if node is not source:
                node.predecessor = None
                node.d = float("inf")
                pair = NodeDistancePair(node, node.d)
                Q.insert(pair)
                node_to_pair[node] = pair
        ...
        while True:
            ...
            for v in u.node.adj:
                if u.node in S:
                    continue
                if v.d > u.distance + weight(u.node, v):
                    v_pair = node_to_pair[v]
                    v.d = u.distance + weight(u.node, v)
                    v_pair.distance = v.d
                    Q.decrease_key(v_pair)
