...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        if len(self.queue)==self.heap_size:
            self.queue.append(key)
        else:
            self.queue[self.heap_size] = key
    ...
    def decrease_key(self, i, new_key):
        """Decrease value of i th element to new_key"""
        if self.queue[i]>new_key:
            raise ValueError('New key value is greater than old key')

        self.queue[i] = new_key
        while i>0 and self.queue[self.parent(i)] > self.queue[i]:
            self.queue[self.parent(i)] , self.queue[i] = self.queue[i] , self.queue[self.parent(i)]
            i=self.parent(i)
    ...
    def min_heapify(self, i):
        ...
        if l<=self.heap_size-1 and self.queue[l] < self.queue[smallest]:
            smallest = l
        if r<=self.heap_size-1 and self.queue[r] < self.queue[smallest]:
            smallest = r
